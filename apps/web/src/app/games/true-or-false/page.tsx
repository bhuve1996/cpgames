...
import { TrueOrFalseGame } from '@/components/true-or-false-game';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { TRUE_FALSE_PACKS } from '@/lib/true-or-false-data';
...
export default function TrueOrFalsePage() {
  ...
  const activePack = TRUE_FALSE_PACKS.find((p) => p.id === activePackId);

  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      <AnimatedBackground />
      <FloatingEmojis preset="party" />
      <SiteHeader variant="subpage" backHref="/games" backLabel="All games" title="True or False" />
        ...
        {!activePack ? (
          <>
            <PageBanner
              ...
              title="True or False"
              description="Read each statement and call it true or false. Perfect for groups."
              className="animate-fade-in-up !p-5 md:!p-6"
            />
            <div className="space-y-3">
              {TRUE_FALSE_PACKS.map((pack, i) => (
                <Card
                  key={pack.id}
                  className={cn(
                  ...
                  onClick={() => {
                    ...
                    toast.info(`Loading ${pack.title}`, `${pack.statements.length} statements`);
                  }}
                >
          ...
          <TrueOrFalseGame pack={activePack} onExit={() => setActivePackId(null)} />
        )}
      </main>
    </div>
  );
