...
import { NeverHaveIEverGame } from '@/components/never-have-i-game';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { NHIE_PACKS } from '@/lib/never-have-i-data';
...
export default function NeverHaveIEverPage() {
  ...
  const activePack = NHIE_PACKS.find((p) => p.id === activePackId);

  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      <AnimatedBackground />
      <FloatingEmojis preset="party" />
      <SiteHeader variant="subpage" backHref="/games" backLabel="All games" title="Never Have I Ever" />
        ...
        {!activePack ? (
          <>
            <PageBanner
              ...
              description="Read each prompt aloud. Tap if you've done it — most fingers down wins."
              className="animate-fade-in-up !p-5 md:!p-6"
            />
            <div className="space-y-3">
              {NHIE_PACKS.map((pack, i) => (
                <Card
                  key={pack.id}
                  className={cn(
                  ...
                  onClick={() => { setActivePackId(pack.id); toast.info(pack.title, `${pack.prompts.length} prompts`); }}
                >
          ...
          <NeverHaveIEverGame pack={activePack} onExit={() => setActivePackId(null)} />
        )}
      </main>
    </div>
  );
