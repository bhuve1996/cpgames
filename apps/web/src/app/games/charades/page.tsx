...
import { CharadesGame } from '@/components/charades-game';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { CHARADES_PACKS } from '@/lib/charades-data';
...
export default function CharadesPage() {
  ...
  const activePack = CHARADES_PACKS.find((p) => p.id === activePackId);

  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      <AnimatedBackground />
      <FloatingEmojis preset="party" />
      <SiteHeader variant="subpage" backHref="/games" backLabel="All games" title="Charades" />
        ...
        {!activePack ? (
          <>
            <PageBanner
              ...
              title="Charades"
              description="Reveal the prompt, act it out silently, and race the 60-second timer."
              className="animate-fade-in-up !p-5 md:!p-6"
            />
            <div className="space-y-3">
              {CHARADES_PACKS.map((pack, i) => (
                <Card
                  key={pack.id}
                  className={cn(
          ...
          <CharadesGame pack={activePack} onExit={() => setActivePackId(null)} />
        )}
      </main>
    </div>
  );
